...
async function seedData() {
  try {
    // Connect to MongoDB
    const MONGO_URI = process.env.MONGO_URI || 'mongodb://localhost:27017/leads-app';
    await mongoose.connect(MONGO_URI);
    console.log('Connected to MongoDB');

    // Check if company already exists
    const existingCompany = await Company.findOne({ companyId: 'COMP00000001' });
    if (existingCompany) {
    ...
    console.log('Sample company created successfully with campaigns:', sampleCompany.campaigns.map(c => c.name));
