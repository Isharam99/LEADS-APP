...
useEffect(() => {
  ...
  async function fetchCampaign() {
    try {
      const res = await fetch(`/api/campaign?campaignId=${CAMPAIGN_ID}&companyId=${COMPANY_ID}`);
...
function getFieldProperties(param: string) {
  const fieldMap: Record<string, { label: string; placeholder: string; type: string; required: boolean }> = {
    'first-name': { label: 'First Name', placeholder: 'John', type: 'text', required: true },
    'last-name': { label: 'Last Name', placeholder: 'Doe', type: 'text', required: true },
    'email': { label: 'Email', placeholder: 'john@example.com', type: 'email', required: true },
    'phone': { label: 'Phone', placeholder: '+1 555 000 0000', type: 'tel', required: false },
    'location': { label: 'Location', placeholder: 'City, Area or Address', type: 'text', required: false },
    'full-name': { label: 'Full Name', placeholder: 'John Doe', type: 'text', required: true },
    'name': { label: 'Name', placeholder: 'John Doe', type: 'text', required: true },
    'phoneNumber': { label: 'Phone Number', placeholder: '+1 555 000 0000', type: 'tel', required: false },
    'whatsapp': { label: 'WhatsApp', placeholder: '+1 555 000 0000', type: 'tel', required: false }
  };
  
  return fieldMap[param] || { label: param, placeholder: `Enter ${param}`, type: 'text', required: false };
}
...
function renderFormFields() {
  ...
  return campaign.params.map((param, index) => {
    ...
    return (
      <div key={param} className={`sm:col-span-1 ${!isEven && campaign.params.length % 2 === 1 && index === campaign.params.length - 1 ? 'sm:col-span-2' : ''}`}>
...
{!mounted || campaignLoading ? "Loading campaign..." : campaign ? `Add a new lead to ${campaign.name}` : "Add a new lead to campaign CAMP00001"}
</p>
</div>
...
<div className="text-sm text-foreground/70">Loading form fields...</div>
</div>
) : (
renderFormFields()
)}
